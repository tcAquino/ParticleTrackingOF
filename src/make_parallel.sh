...
then
  ...
  make "BUILD=$BUILD" "PTHBIN_BASE=$PTHBIN_BASE" ParticleTrackingOF_Parallel
  if [[ $? -eq 0 ]]
  then
  ...
  ./set_model_parallel.sh $1
  make "BUILD=$BUILD" "PTHBIN_BASE=$PTHBIN_BASE" ParticleTrackingOF_TwoPhaseNonStationary_Parallel
  if [[ $? -eq 0 ]]
  then

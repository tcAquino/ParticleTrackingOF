...
namespace useful
{
  /** \class MultiArray General/MultiArray.h "General/MultiArray.h"
   ...
   \details Different dimensions may have different sizes. The elements are indexed, traversing the container in the order (0,...,0), (0,...,1), ..., (0,..,1,0) in either 1D or using a multi-dimensional index. The elements are continguous in memory in the order of the 1D index.
  */
    ...
    */
    ...
    /** \return Value at 1D index \p index, traversing the container in the order (0,...,0), (0,...,1), ..., (0,..,1,0), etc.  */
		value_type const& operator[] (std::size_t index) const
		{
			return _values[index];
		}
    
    /** \return Value at 1D index \p index, which traverses the container in the order (0,...,0), (0,...,1), ..., (0,..,1,0), ....  */
		value_type& operator[] (std::size_t index)
		{
			return _values[index];
		}

    /** \return Convert multi-dimensional index \p indexes to 1D index, which traverses the container in the order (0,...,0), (0,...,1), ..., (0,..,1,0), ....  */
		std::size_t computeIndex(std::vector<std::size_t> const& indexes) const
		{
      return modular::positionToIndex(indexes, _dimensions);
		}

    /** \return Convert 1D index \p index, which traverses the container in the order (0,...,0), (0,...,1), ..., (0,..,1,0), ..., to multi-dimensional index.  */
    ...
    /** \brief Convert 1D index \p index to multi-dimensional index.
     \param index Index to convert, which traverses the container in the order (0,...,0), (0,...,1), ..., (0,..,1,0).
     \param indexes Multidimensional index is returned here.
    ...
    /** \return Iterator to beginning. Container is traverse in the order (0,...,0), (0,...,1), ..., (0,..,1,0). */
		auto begin()
		{ return values.begin(); }

    /** \return Iterator to end. Container is traverse in the order (0,...,0), (0,...,1), ..., (0,..,1,0). */
		auto end()
		{ return values.end(); }
    ...
    /** \return Constant iterator to beginning. Container is traverse in the order (0,...,0), (0,...,1), ..., (0,..,1,0). */
		auto cbegin() const
		{ return values.cbegin(); }

    /** \return Constant iterator to end. Container is traverse in the order (0,...,0), (0,...,1), ..., (0,..,1,0). */
		auto cend() const
		{ return values.cend(); }

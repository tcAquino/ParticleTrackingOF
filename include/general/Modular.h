...
namespace modular
{
  ...
  /** \return Base \p basis representation of base-10 integer \p to_convert as a string of digits.
  \note Units digit in  \p to_convert[0]. */
  template <typename Integer_Type>
  std::vector<Integer_Type> convert
  (Integer_Type to_convert, Integer_Type basis)
  {
  ...
  /** \return Base \p basis representation of base-10 integer \p to_convert as a string of digits.
   \details The total number of digits is \p nr_digits, which must be at least the number of digits needed to represent the number.  If bigger, the result is padded with zeros on the left.
   \note Units digit in \p to_convert[0]. */
  template <typename Integer_Type>
  std::vector<std::size_t> convert
  (Integer_Type to_convert, Integer_Type basis, std::size_t nr_digits)
  {
  ...
  /** \brief  Given 1d index \p idx, convert to position (x_idx,y_idx,...) in a regular grid; output in \p position.
  \details \p idx goes through every x for each value of y, every y for each value of z, etc, in ascending order
  ...
  /** \retval idx 1d index corresponding to position (x_idx,y_idx,...) in a regular grid.
  \details \p idx goes through every x for each value of y, every y for each value of z, etc, in ascending order */
  template <typename Container>
  ...
  /** \brief \p num are the digits of a number such that each digit ranges from 0 to \p basis[dig] - 1. Increment it by 1.
   \note
   - Units digit in \p num[0].
   - Maximum value wraps to 00....0. */
  template <typename Container>
  ...
  /** \brief \p num are the digits of a number such that each digit ranges from 0 to \p basis[dig] - 1. Increment it by \p inc.
  \note
  - Units digit in \p num[0].
  - Maximum value wraps to 00....0. */
  template <typename Container>

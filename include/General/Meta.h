...
namespace meta
{
  ...
  template <typename X>
  using has_begin = has_member<begin_result_t, X>;
  template <typename X>
  /** \brief Check if a class has method \c begin(). */
  ...
  template <typename X>
  using size_result_t = decltype(std::declval<X>().size());
  /** \brief Check if a class has method \c size(). */
  template <typename X>
  using has_size = has_member<size_result_t, X>;
  /** \brief Check if a class has method \c size(). */
  template <typename X>
  inline constexpr bool has_size_v = has_size<X>::value;
  ...
  template <typename X>
  using has_reserve = has_member<reserve_result_t, X>;
  template <typename X>
  /** \brief Check if a class has method \c reserve(). */
  ...
  template <typename X>
  using has_push_back = has_member<push_back_result_t, X>;
  template <typename X>
  /** \brief Check if a class has method \c push_back(). */
  ...
  // The following has_operator methods and associated machinery are adapted from Richard Hodges's answer here:
  // https://stackoverflow.com/questions/51404763/c-compile-time-check-that-an-overloaded-function-can-be-called-with-a-certain .
  ...
  { static constexpr bool value = std::is_constructible_v<X,
      decltype(std::declval<X>().size())>; };
  /** \brief Check if \c X<Y>(size()) is viable. */
  template <typename X>
  ...
  { static constexpr bool value = std::is_convertible_v<X,
      decltype(std::declval<Y>()*std::declval<Z>())>; };
  ...
  { static constexpr bool value = std::is_convertible_v<X,
    decltype(std::abs(std::declval<Y>()))>; };
  /** \brief Check if \c X=std::abs(Y) iis viable. */
  template <typename X, typename Y>
  ...
  { static constexpr bool value = std::is_convertible_v<X,
      decltype(std::sqrt(std::declval<Y>()))>; };
  /** \brief Check if \c X=std::sqrt(Y) is viable. */
  template <typename X, typename Y, typename Z>

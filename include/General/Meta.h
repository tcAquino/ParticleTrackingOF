...
/** @brief Check whether std::abs() can be called on type. */
template <typename... Args>
...
/** @brief Check if <tt>Z = X * Y</tt> is viable. */
...
/**  @brief Check if <tt>Y = std::abs(X)</tt> is viable. */
...
/**
 * @brief Check if <tt>Z = std::pow(X, Y)</tt> is viable.
 */
template <typename X, typename Y, typename Z>
...
/**
 * @brief Check if <tt>Y = std::sqrt(X)</tt> is viable.
 */

 ...
 *
 * @note Units digit in <tt>to_convert[0]</tt>.
 */
 ...
 *        digits.
 * @note Units digit in <tt>converted[0]</tt>.
 */
 ...
 *          the number of digits needed to represent the number. If bigger, the
 *          result is padded with zeros on the left.
 *
 * @note Units digit in <tt>to_convert[0]</tt>.
 */
template <typename Integer_Type>
 ...
 *
 * @note \p position container must be passed with the correct size.
 */
 ...
 *         index \p idx.
 *
 * @details \p idx goes through every x for each value of
 *          y, every y for each value of z, etc, in ascending order.
 */
template <typename Container, typename IntegerType = std::size_t>

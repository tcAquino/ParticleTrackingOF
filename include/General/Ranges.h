...
namespace range
{
  ...
  Container<Scalar, Args...> linspace(Scalar xx1, Scalar xx2, std::size_t nr_points)
  {
    if (nr_points == 0)
      return {};
  ...
  template <template <typename...> typename Container = std::vector,
  typename Scalar, typename... Args>
  Container<Scalar, Args...> logspace(Scalar xx1, Scalar xx2, std::size_t nr_points)
  {
    if (nr_points == 0)
      return {};
  ...
  /** \return Linear partition of \c xx1 up to \c xx2 (excluded), with spacing given by \p increment.
   \note For signed \c Scalar types, the last element can be smaller than the first. The sign of the increment is adjusted automatically */
  template <template <typename...> typename Container = std::vector,
  typename Scalar, typename... Args>

...
namespace ctrw
{
  /** \class CTRW_Parallel CTRW/CTRW_Parallel.h "CTRW/CTRW_Parallel.h"
   ...
   \details Particles undergo transitions handled by a separate class passed to the evolution methods.
  \tparam State_t Characterizes a particle's state.
   ...
   - Particle evolution methods expect a container of Transitions objects, one per parallel thread.
   */
  ...
  public:
    ...
    */
    ...
    void evolve_time
    (double time_to, TransitionsContainer&& transitions)
    {
      evolve
    ...
    */
    template <typename TransitionsContainer>
    void evolve_space
    (double length_to, TransitionsContainer&& transitions)
    {
      evolve
    ...
    */
    template
    <typename Criterion, typename TransitionsContainer>
    void evolve
    (Criterion&& criterion, TransitionsContainer&& transitions)
    {
  ...
  private:
    ...
    { if (criterion(_particles[part])) remove(part); }
    
    /** \brief Remove particle.
     \param part Index of particle to remove.
     */
    void remove(std::size_t part)
    { useful::swap_erase(_particles, part); }
  };
}

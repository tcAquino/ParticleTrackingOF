...
namespace geom
{
  /**
   ...
   \return Displacement to be added to \p position to place it within periodic boundaries. */
  ...
  /**
   \param position 1D position .
   \param boundaries Pair of lower and upper boundary location.
   \return Pair of displacement to be added to \p position and the associated signed number of domain displacements out
  ...
  \param position 1D position.
  \param boundaries Pair of lower and upper boundary location.
  \return Displacement to be added to 1D \p position to place it within reflecting boundaries. */
   ...
   * - \c position [scalar] */
	...
	{
	public:
		const std::pair<double, double> boundaries;  /**< Lower and upper boundary location. */
   ...
   * - \c position [container] */
	...
	{
	public:
		const std::vector<std::pair<double, double>> boundaries; /**< Lower and upper boundary locations along each dimension. */
    const std::vector<double> domain_dimensions; /**< Size of domain along each dimension. */
  ...
  public:
    ...
    const std::pair<double, double> boundaries;  /**< Lower and upper boundary location along dimension \c dd. */
    static const std::size_t dim = dd;           /**< Cartesian dimension along which BC holds. */
    ...
    bool operator()(State& state, State const& state_old) const
    {
      ...
      while (1)
      {
        ...
        if (alpha <= 0. || std::isnan(alpha))
        {
           ...
           for (std::size_t dd = 0; dd < 2; ++dd)
             state.position[dd+_begin_transverse] *= radius/std::sqrt(radial_pos_sq);
           break;
        }
  ...
  private:
    const double _radius_sq{ radius*radius }; /**< Domain radius along reflecting dimensions squared. */
    const std::size_t _begin_transverse{ dd_open == 0 ? 1 : 0 };  /** First reflecting dimension. */
  };
  ...
  {
  public:
    const SymmetryPlanes symmetry_planes;   /**> Object describing domain symmetry planes (see Geometry/SymmetryPlanes.h). */
    const double scale;                     /**> Scale factor. */
    const std::vector<double> origin;       /**> Coordinate origin. */
    
    /** Constructor.
     \param symmetry_planes Object describing domain symmetry planes (see Geometry/SymmetryPlanes.h).

/**
 * @file   SymmetryPlanes.h
 * @author Tomás Aquino <tomas.aquino@csic.es>
 * @date   Wed May 13 00:00:00 2020
 *
 * @brief Symmetry planes to describe general symmetries.
 *
 * @details A SymmetryPlanes object must define:
 *
...
struct SymmetryPlanes_Bcc {
  ...
  SymmetryPlanes_Bcc()
      : normal{op::times_scalar(1. / std::sqrt(2.),
        ...
        translation{op::times_scalar(2. / std::sqrt(3.),
  ...
  const std::vector<double> length{std::vector<double>(
      dim, 4. / std::sqrt(6.))}; /**< Primitive cell side. */
  const std::vector<std::vector<double>>
      normal; /**< Unit normal vectors associated with each plane. */
  const std::vector<std::vector<double>>
 ...
 *
 * @note \c Boundary_Periodic must define:
 *
 * - \c symmetry_planes
 *
 * - \c scale
...
void add_periodic_images(Points &points, Velocities &velocities,
                         double fraction,
  ...
  std::vector<std::vector<int>> projections_backward(dim,
                                                     std::vector<int>(dim));
  ...
  for (std::size_t pp = 0; pp < nr_points; ++pp) {
    auto projections =
        project(points[pp], boundary_periodic.symmetry_planes,
                boundary_periodic.scale, boundary_periodic.origin);
    for (std::size_t dd = 0; dd < dim; ++dd) {

...
auto uniform_solid_reactant(double surface_concentration,
                            Container const &face_ids, Mesh const &mesh) {
...
auto uniform_solid_reactant_patches(double surface_concentration,
                                    std::vector<std::string> const &patch_names,
                                    Mesh const &mesh) {
  return uniform_solid_reactant(surface_concentration,
                                patches_face_ids(mesh, patch_names), mesh);
}
 ...
 * "PTOF/Reaction.h" \brief A_F + A_S -> A_S surface reaction. */
...
public:
  ...
  /** Constructor.
    ...
    \param surface_concentrations Map of initial reactive surface concentrations
    over mesh faces. */

...
public:
  ...
  OutputParameters(Directories const &directories, std::string const &name,
    ...
    read_measure_spacing(input, params_transport, params_reaction);
  ...
  void read_time_units(IStream &input,
    ...
    default:
      throw std::runtime_error{std::string("Measure spacing units ") +
  ...
  template <typename IStream, typename TransportParameters,
            typename ReactionParameters>
  void read_measure_spacing(IStream &input,
                            TransportParameters const &params_transport,
                            ReactionParameters const &params_reaction) {
    ...
    case MeasureSpacing::Type::step: {
      ...
      if (!(time_increment > 0.))
        throw std::runtime_error{
            std::string("Non-positive time increment for ") + measure_spacing +
            " measurement spacing"};
      break;
    }
    ...
    default:
      throw std::runtime_error{std::string("Measure spacing ") +
                               measure_spacing + " not supported"};
    }
  }

  /** \brief Read measure spacing type from input stream. */
  template <typename IStream> void read_measurement_types(IStream &input) {
    std::string line;
...
public:
  ...
  /** Constructor.
   ...
   assigned values to mesh cells through operator[]. \param thresholds Vector of
   thresholds for each mask, such that cells where a mask is above the threshold
   are considered.
  */
  ...
  Output_Cases(Subject const &subject, VelocityField const &velocity_field,
    ...
    set_measure_types(subject, geometry, directories, identifier,
  ...
  */
  ...
  Output_Cases(Subject const &subject, Geometry const &geometry,
               Directories const &directories, Parameters parameters,
               std::string const &identifier,
               std::vector<std::reference_wrapper<const Mask>> masks = {},
               std::vector<double> thresholds = {})
      : Output_Cases(subject, useful::Empty{}, geometry, directories,
                     parameters, identifier, masks, thresholds) {
    set_measure_types(directories, identifier, useful::Empty{}, masks,
                      thresholds);
    set_end_criterion();
    set_next_measure_time();
  }
  ...
  */
  ...
  void set_next_measure_time() {
    ...
    case MeasureSpacing::Type::step: {
      _next_measure = std::make_unique<NextMeasureTime_step>(parameters);
      break;
    }
    case MeasureSpacing::Type::linear: {
      _next_measure = std::make_unique<NextMeasureTime_linear>(parameters);
      break;
    }
    case MeasureSpacing::Type::log: {
      _next_measure = std::make_unique<NextMeasureTime_log>(parameters);
      break;
    }
    default:
      throw std::runtime_error{"Measure spacing " + parameters.measure_spacing +
                               " not supported"};
    }
  }

  /** \return Time of next measurement. */
  double next_measure_time() const { return _next_measure->time(); }
...
private:
  ...
  void info_runtime_measurements(OStream &output,
    ...
    int width_name =
        int(std::max_element(parameters.measurements.begin(),
    ...
    int width_field = int(std::max_element(parameters.measurements.begin(),
                                           parameters.measurements.end(),
                                           [](auto const &aa, auto const &bb) {
                                             return aa.field_name.length() <
                                                    bb.field_name.length();
                                           })
                              ->field_name.length()) +
                      1;
    for (auto const &measurement : parameters.measurements) {
      output << prefix << std::left << std::setw(width_name)
             << measurement.name;
      if (!measurement.field_name.empty())
        output << std::left << std::setw(width_field) << measurement.field_name;
      output << "\n";
    }
  }
  ...
  void
  set_measure_types(Subject const &subject, Geometry const &geometry,
  ...
  protected:
    /** Constructor. */
    NextMeasureTime(Parameters const &parameters)
  ...
  struct NextMeasureTime_step final : NextMeasureTime {
    NextMeasureTime_step(Parameters const &parameters)
  ...
  struct NextMeasureTime_linear final : NextMeasureTime {
    NextMeasureTime_linear(Parameters const &parameters)
        : NextMeasureTime{parameters} {}
  ...
  struct NextMeasureTime_log final : NextMeasureTime {
    NextMeasureTime_log(Parameters const &parameters)
        : NextMeasureTime{parameters} {}
  ...
  std::unique_ptr<NextMeasureTime>
      _next_measure; /**< Handle next measure time. */

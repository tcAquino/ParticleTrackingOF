...
namespace ptof
{
  ...
  struct MeasurerTime_position_in_regions final : MeasurerTime<Subject, Geometry>
  {
    ...
    : MeasurerTime<Subject, Geometry>{ subject,
        geometry, directories,
        "position_in_regions", identifier,
        precision }
    , _masks{ masks }
  ...
  template <typename Subject, typename Geometry>
  struct MeasurerTime_position_second_moment final : MeasurerTime<Subject, Geometry>
  {
    MeasurerTime_position_second_moment
    (Subject const& subject,
     Geometry const& geometry,
     Directories const& directories,
     std::string const& identifier,
    ...
    : MeasurerTime<Subject, Geometry>{ subject,
        geometry, directories,
        "position_second_moment", identifier,
        precision }
    , _column_widths{
      std::max(9 + precision, int(1 + std::string{ "Time" }.length())),
    ...
    void operator()(double time) override
    {
      _output << std::setw(_column_widths[0]) << time;
      useful::print(_output,
                    position_second_moment(_subject, time),
                    _column_widths[1]);
      _output << "\n";
    }
  ...
  {
    MeasurerTime_position_variance
    (Subject const& subject,
     Geometry const& geometry,
     Directories const& directories,
     std::string const& identifier,
    ...
    void operator()(double time) override
    {
      _output << std::setw(_column_widths[0]) << time;
      useful::print(_output,
                    position_variance(_subject, time),
                    _column_widths[1]);
      _output << "\n";
    }
  ...
  struct MeasurerTime_mass_in_regions final : MeasurerTime<Subject, Geometry>
  {
    ...
    void operator()(double time) override
    {
      _output << std::setw(_column_widths[0]) << time;
      useful::print(_output,
                    mass(_subject, time, _locator, _masks, _thresholds),
                    _column_widths[1]);
      _output << "\n";
    }
  ...
  */
  ...
  struct MeasurerTime_scalar_field final : MeasurerTime<Subject, Geometry>
  {
    ...
    (Subject const& subject,
     ...
     std::string const& field_name,
     int precision = 8)
    : MeasurerTime_scalar_field{ subject,
        ScalarField_LinearInterpolation_OF
        <Foam::volScalarField, typename Geometry::Locator const&, CheckOptions::Check>{
  ...
  (Subject const&,
   ...
   std::string const&,
   int) ->
  MeasurerTime_scalar_field<Subject, Geometry,
  ScalarField_LinearInterpolation_OF
  <Foam::volScalarField, typename Geometry::Locator const&, CheckOptions::Check>>;
  template <typename Subject, typename Geometry>
  MeasurerTime_scalar_field
  ...
  struct MeasurerTime_vector_field final : MeasurerTime<Subject, Geometry>
  {
    ...
    (Subject const& subject,
     ...
     std::string const& field_name,
     int precision = 8)
    : MeasurerTime_vector_field{ subject,
        VectorField_LinearInterpolation_OF
        <Foam::volVectorField, typename Geometry::Locator const&, CheckOptions::Check>{
          { Foam::IOobject{ field_name,
            geometry.mesh().time().timeName(),
  ...
  (Subject const&,
   ...
   std::string const&,
   int) ->
  MeasurerTime_vector_field<Subject, Geometry,
  VectorField_LinearInterpolation_OF
  <Foam::volVectorField, typename Geometry::Locator const&, CheckOptions::Check>>;
  template <typename Subject, typename Geometry>
  MeasurerTime_vector_field
  ...
  template <typename Subject, typename Geometry, typename Field = Foam::volTensorField>
  struct MeasurerTime_tensor_field final : MeasurerTime<Subject, Geometry>
  {
    ...
    (Subject const& subject,
     ...
     std::string const& field_name,
     int precision = 8)
    : MeasurerTime_tensor_field{ subject,
  ...
  {
    MeasurerTime_position_periodic
    (Subject const& subject,
     Geometry const& geometry,
     Directories const& directories,
     std::string const& identifier,
    ...
    void operator()(double time) override
    {
      ...
      for (auto const& part : _subject.particles())
      {
        ...
        {
          _output << std::setw(_column_widths[1]) << state.tag;
          useful::print(_output, _getter_position(state),
                        _column_widths[2]);
          _output << std::setw(_column_widths[3]) << state.mass;
        }
      }
  ...
  /** \class MeasurerTime_position_in_regions_periodic PTOF/Output.h "PTOF/Output.h"
   *  \brief  Output time, tags, positions, and masses within regions specified by masks. */
  template <typename Subject, typename Geometry, typename Mask>
  struct MeasurerTime_position_in_regions_periodic final : MeasurerTime<Subject, Geometry>
  {
    ...
    : MeasurerTime<Subject, Geometry>{ subject,
        geometry, directories,
        "position_in_regions_periodic", identifier,
        precision }
    , _getter_position{ geometry.boundary_periodic }
    , _masks{ masks }
    , _thresholds{ thresholds }
  ...
  {
    MeasurerTime_position_mean_periodic
    (Subject const& subject,
     Geometry const& geometry,
     Directories const& directories,
     std::string const& identifier,
    ...
    : MeasurerTime<Subject, Geometry>{ subject,
        geometry, directories,
        "position_mean_periodic", identifier,
        precision }
  ...
  /** \class MeasurerTime_position_second_moment_periodic PTOF/Output.h "PTOF/Output.h"
  ...
  : MeasurerTime<Subject, Geometry>
  {
    MeasurerTime_position_second_moment_periodic
    (Subject const& subject,
     Geometry const& geometry,
     Directories const& directories,
     std::string const& identifier,
    ...
    : MeasurerTime<Subject, Geometry>{ subject,
        geometry, directories,
        "position_second_moment_periodic", identifier,
        precision }
    , getter_position{ geometry.boundary_periodic }
    , _column_widths{
      std::max(9 + precision, int(1 + std::string{ "Time" }.length())),
  ...
  : MeasurerTime<Subject, Geometry>
  {
    MeasurerTime_position_variance_periodic
    (Subject const& subject,
     Geometry const& geometry,
     Directories const& directories,
     std::string const& identifier,
    ...
    : MeasurerTime<Subject, Geometry>{ subject,
        geometry, directories,
        "position_variance_periodic", identifier,
        precision }
    , getter_position{ geometry.boundary_periodic }
    , _column_widths{
      std::max(9 + precision, int(1 + std::string{ "Time" }.length())),

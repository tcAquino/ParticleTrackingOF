 ...
 * "PTOF/Steppers.h" \brief Time steppers for RK4 advection and stochastic
...
struct Steppers_Advection_RK4_Diffusion_Euler {
  ...
  */
  ...
  /**
   ...
   If \p params_solvers does not define \c time_step, the time step is initially
   set to zero.
   */
  ...
  makeJumpGenerator(VelocityField &&velocity_field, Boundary &&boundary,
    ...
    if constexpr (meta::has_time_step_v<SolverParameters>)
      return ctrw::JumpGenerator_Add{
          ...
          ctrw::JumpGenerator_Diffusion{params_transport.diff_coeff,
                                        params_solvers.time_step, dim}};
    return ctrw::JumpGenerator_Add{
        ctrw::JumpGenerator_Velocity_RK4{
            std::forward<VelocityField>(velocity_field), 0.,
            std::forward<Boundary>(boundary)},
        ctrw::JumpGenerator_Diffusion{params_transport.diff_coeff, 0., dim}};
  }
...
/** \struct Steppers_Advection_Euler_Diffusion_Euler PTOF/Steppers.h
 "PTOF/Steppers.h" \brief Time steppers for forward Euler advection and
 stochastic forward Euler diffusion. */

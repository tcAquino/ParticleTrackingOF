...
template <typename Particle, typename Locator> struct ParticleMaker {
  ...
  ParticleMaker(meta::Selector_t<Particle>, Locator &&locator, Time time = {},
                Mass mass = {}, Tag tag = {}, Info info = {})
      : locator{std::forward<Locator>(locator)}, time{time}, mass{mass},
        tag{tag} {}

  template <typename Position> Particle operator()(Position const &position) {
    return {make_state(State::make_position(position), info, locator, time,
                       mass, tag)};
  }
...
ParticleMaker(meta::Selector_t<Particle>, Locator &&locator,
...
struct ParticleMaker_Periodic {
  ...
  template <typename Position> Particle operator()(Position const &position) {
    ...
    typename Particle::State state{
        make_state(state_position, std::vector<int>(state_position.size(), 0),
